...
def test(data_loader, Net, loss_fn, log_file, vis_path, testing_score, task):
	...
	print_with_write(log_file ,'[---: ---/---] %10s loss: %f OverallAcc: %f MeanAcc %f mIoU %f fps: %f'%(('validation'), testing_loss, score['OverallAcc'], score['MeanAcc'], score['mIoU'],(time_testing.shape)/time_testing.sum()))
...
if __name__ == '__main__':
	...
	parser.add_argument('-m', '--model-ckpt',  type=str,            default='.//log//savemodel//model_60.pth',       help='the file name of checkpoint you want to test')
